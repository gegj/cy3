...
class InviteDB {
    constructor() {
        ...
        // 添加刷新操作锁，防止多次同时执行刷新操作
        this._refreshOperationLock = false;
    }
    ...
    async refreshAddUsers() {
        // 如果已经有刷新操作在执行，返回空结果
        if (this._refreshOperationLock) {
            console.warn('已有刷新操作在执行中，跳过本次刷新');
            return { increment: 0, newInvites: [] };
        }
        
        // 设置刷新操作锁
        this._refreshOperationLock = true;
        console.log('刷新操作开始，设置锁');
        ...
        try {
            ...
            setTimeout(() => {
                this._refreshOperationLock = false;
                console.log('刷新操作结束，释放锁');
            }, 500); // 延迟500ms释放锁，防止极端情况下的连续调用
        }
    }
